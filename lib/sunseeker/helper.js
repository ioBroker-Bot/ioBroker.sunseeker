module.exports = {
    ...
    async addDeleteObject(sn, data, objChannel, objName, lang) {
        ...
        const obj = objs.filter(m => m._id.includes(`${this.namespace}.${sn}.map.${objChannel}.0`));
        ...
        for (let a = 1; a <= zone; a++) {
            ...
            await this.iob
                .setObjectNotExistsAsync(`${this.namespace}.${path}.delete_${objName}`, {
                    type: "state",
                    common: {
                        name: lang,
                        type: "boolean",
        ...
        if (zones > zone) {
            ...
            for (let a = zone; a <= zones - 1; a++) {
                ...
                this.log.info(`delete ${objName}: ${this.namespace}.${sn}.map.${objChannel}.${counter}`);
                await this.iob.delObjectAsync(`${this.namespace}.${sn}.map.${objChannel}.${counter}`, {
                    recursive: true,
                });
                --count;
                ++save;
    ...
    async setStates(sn, data, isAvailable) {
        ...
        if (data.time_zone != null || data.wirelessTimeZone != null) {
