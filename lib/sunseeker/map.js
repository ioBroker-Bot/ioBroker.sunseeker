...
module.exports = {
    ...
    async renderLivemapInner(PImage, mapData, pathData, sn, meta) {
        ...
        const groups = ["region_work"];
        if (settings.divide_area_work_active) {
            groups.push("divide_area_work");
        }
        if (settings.region_channel_active) {
            groups.push("region_channel");
        }
        if (settings.region_obstacle_active) {
            groups.push("region_obstacle");
        }
        if (settings.region_forbidden_active) {
            groups.push("region_forbidden");
        }
        if (settings.region_placed_blank_active) {
            groups.push("region_placed_blank");
        }
        ...
        if (settings.region_forbidden_active) {
            for (const pts of collected.region_forbidden) {
                if (pts.type === "dark") {
                    drawPoly(
                        pts,
                        settings.region_forbidden_dark_fill,
                        settings.region_forbidden_dark_stroke,
                        settings.region_forbidden_dark_lineWidth,
                    );
                } else if (pts.type === "normal" && pts.tag == 1) {
                    drawPoly(
                        pts,
                        settings.region_forbidden_wall_fill,
                        settings.region_forbidden_wall_stroke,
                        settings.region_forbidden_wall_lineWidth,
                    );
                } else {
                    drawPoly(
                        pts,
