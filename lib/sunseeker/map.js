...
module.exports = {
    ...
    async fetchMap(sn) {
        ...
        if (!meta || (meta.modelClass !== "S" && meta.modelClass !== "X")) {
            this.log.debug(`fetchMap ${sn}: Skip modelclass ${meta && meta.modelClass} without maps`);
            return;
        }
        if (meta._mapInFlight) {
            this.log.debug(`fetchMap ${sn}: is running`);
            return;
        }
        meta._mapInFlight = true;
        this.log.debug(`fetchMap ${sn}: Request map infos`);
        try {
            ...
            if (!data) {
                this.log.debug(`fetchMap ${sn}: No map.info data`);
                return;
            }
            ...
            if (newMapId !== undefined) {
                if (mapChanged) {
                    ...
                    this.log.debug(`fetchMap ${sn}: mapModifyTime ${newMapId} unchanged`);
                }
            }
    ...
    async fetchMapWithMqttData(sn) {
        ...
        const meta = this.deviceMeta[sn];
        if (!meta || (meta.modelClass !== "S" && meta.modelClass !== "X")) {
            this.log.debug(`fetchMapMqtt ${sn}: Skip modelclass ${meta && meta.modelClass} without maps`);
            return;
        }
        if (meta._mapMqttInFlight) {
            this.log.debug(`fetchMapMqtt ${sn}: is running`);
            return;
        }
        meta._mapMqttInFlight = true;
        this.log.debug(
            `fetchMapMqtt ${sn}: Livemap rendern (mapData=${!!meta.mapJson} pathData=${!!meta.pathJson} live=${meta.livePath ? meta.livePath.length : 0})`,
        );
        const dataUrl = await this.renderLivemap(meta.mapJson, meta.pathJson, meta);
        if (dataUrl) {
            this.emit("livemap", { sn, dataUrl });
            this.log.debug(`fetchMapMqtt ${sn}: Livemap emitted (${dataUrl.length} Bytes data URL)`);
        } else {
            this.log.debug(`fetchMapMqtt ${sn}: Livemap not rendered (no geometry)`);
        }
    ...
    async renderLivemapInner(PImage, mapData, pathData, meta) {
        ...
        const robotSprite = await loadSprite(PImage, "robot.png", _robotSprite).catch(() => null);
        const chargerSprite = await loadSprite(PImage, "charger.png", _chargerSprite).catch(() => null);
        const spriteScale = (canvasW + canvasH) / 2 / 1000;
