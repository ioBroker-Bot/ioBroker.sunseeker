...
const NEW_ACTIONS = {
    ...
    passages: { cmd: "draw_passage", cmdid: "drawPassage" },
};
...
module.exports = {
     ...
     * @param {string} sn
     */
    async getDeviceSkin(sn) {
        const dev = this.devicesRaw[sn];
        if (!dev) {
            this.iob.log.error(`Device ${sn} unknown!`);
            return;
        }
        const apiPath = `/app_wireless_mower/device-skin?userId=${String(dev.appUserId || this.session.user_id)}`;
        const res = await this.request("GET", apiPath, {
            ...this.authHeaders(),
            "Content-Type": "application/json; charset=UTF-8",
        });
        if (res.json && res.json.ok === false) {
            this.iob.log.error(`API Skin: ${res.json.msg}`);
        } else {
            this.iob.log.debug(`getDeviceSkin: ${JSON.stringify(res.json)}`);
            //ToDo Emit
        }
    },
    /**
     ...
     * @param {string} sn
     */
    async getCommunityActivities(sn) {
        const dev = this.devicesRaw[sn];
        if (!dev) {
            this.iob.log.error(`Device ${sn} unknown!`);
            return;
        }
        const apiPath = `/app_wireless_mower/community_activities/${String(dev.appUserId || this.session.user_id)}`;
        const res = await this.request("GET", apiPath, {
            ...this.authHeaders(),
            "Content-Type": "application/json; charset=UTF-8",
        });
        if (res.json && res.json.ok === false) {
            this.iob.log.error(`API Comminity: ${res.json.msg}`);
        } else {
            this.iob.log.debug(`getCommunityActivities: ${JSON.stringify(res.json)}`);
            //ToDo Emit
        }
    },
    /**
     ...
     * @param {string} sn
     */
    async getAgreements(sn) {
        const dev = this.devicesRaw[sn];
        if (!dev) {
            this.iob.log.error(`Device ${sn} unknown!`);
            return;
        }
        const apiPath = `/app_wireless_mower/app-agreements/business-types/1/items`;
        const res = await this.request("GET", apiPath, {
            ...this.authHeaders(),
            "Content-Type": "application/json; charset=UTF-8",
        });
        if (res.json && res.json.ok === false) {
            this.iob.log.error(`API Agreements: ${res.json.msg}`);
        } else {
            this.iob.log.debug(`getAgreements: ${JSON.stringify(res.json)}`);
            //ToDo Emit
        }
    },
    /**
    ...
    /**
     * Currently not in use
     * Response time and custom time. Slots 1 and 2 separate
     * For S and X devices
     *
     * @param {string} sn
     */
