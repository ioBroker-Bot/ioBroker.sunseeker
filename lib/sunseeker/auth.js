...
module.exports = {
    ...
    /**
     *  @returns {{"Accept-Language":string, Authorization: string, Host:string, Connection: string, "User-Agent": string}}
     */
    authHeaders() {
    ...
    async _sendHttp(method, urlPath, headers, data) {
        ...
        this.iob.log.debug(`HTTP ${method} ${urlPath}`);
    ...
    /**
     * @param {import("buffer").WithImplicitCoercion<string>} plaintext
     */
    ...
    /**
     * Create random string length 24
     */
    randomMqttPassword() {
    ...
    async ensureAppId() {
        ...
        try {
            const st = await this.iob.getStateAsync("auth.app_id");
            if (st && typeof st.val === "string" && /^[0-9a-f]{16}$/.test(st.val)) {
                this.appId = st.val;
                return this.appId;
            }
        } catch (err) {
            this.iob.log.debug(`ensureAppId read: ${err.message}`);
        }
        this.appId = crypto.randomBytes(8).toString("hex");
        await this.iob.setStateAsync("auth.app_id", { val: this.appId, ack: true });
        this.iob.log.info(`Generated new MQTT app id: ${this.appId}`);
        return this.appId;
    },
    ...
    async editMqttPassword() {
        ...
        const mqttData = {
            pw: this.mqttPassword,
            key: encrypted,
        };
        this.emit("mqtt_auth", mqttData);
