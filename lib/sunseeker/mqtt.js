...
module.exports = {
    ...
    async startMqttNew() {
        ...
        try {
            ...
            this.iob.log.error(`Setting MQTT password failed: ${err.message}`);
            this.emit("error", err);
            return;
        }
        if (this.unloading) {
    ...
    connectMqtt() {
        ...
        this.iob.log.info(`MQTT connection ${url} username=${username}`);
        ...
        if (meta.livemap && (data.id == "device_pos" || data.id == "getPathData")) {
            ...
            if (!meta._refreshTimer) {
                meta._refreshTimer = this.iob.setTimeout(() => {
                    ...
                    this.updateDevice(sn).catch(err =>
                        this.iob.log.debug(`Refresh after MQTT (${sn}): ${err.message}`),
                    );
                }, this.options.refreshAfterMqttMs);
            } else {
                this.fetchMapWithMqttData(sn).catch(err => this.iob.log.debug(`Map ${sn}: ${err.message}`));
            }
        }
    },
