...
module.exports = {
    ...
    onMqttMessage(topic, payload) {
        ...
        if (data.pos && typeof data.pos.lng === "string") {
            data.pos.lng = parseInt(data.pos.lng);
            data.pos.lat = parseInt(data.pos.lat);
        }
        this.emit("mqtt", { sn, topic, data: statusData });
        this.emit("setMqtt", { sn, topic, data: statusData, id: id });
        ...
        if (meta.livemap && (data.id == "device_pos" || data.id == "getPathData")) {
            if (meta.pathJson && data.id == "getPathData") {
                try {
                    if (typeof statusData.path === "string") {
                        meta.pathJson = meta.pathJson.concat(JSON.parse(statusData.path));
                    } else {
                        this.log.debug(`MQTT PATH NOT PARSE`);
                    }
                } catch (e) {
                    this.log.debug(`MQTT PARSE PATH: ${e}`);
                }
            }
            if (!meta._refreshTimer) {
                ...
                this.fetchMapWithMqttData(sn).catch(err => this.log.debug(`Map ${sn}: ${err.message}`));
            }
        }
    },
